...
- `--exclude-file`
  - 除外するファイルの設定
  - 複数回使うと複数ファイルの指定ができる
  - `{id}/{filename}` 形式で、 id だけの指定ならその Gist の全ファイルが対象
  - デフォルト: `[]`
- `--config` (`-C`)
  - 設定ファイルのパスを指定
  - コマンドライン引数を JSON で指定できる
  - コマンドライン引数の方が優先される
  - デフォルト: ファイルなし

### `_meta.json`

Gist 内の `_meta.json` ファイルを参照  
`search_exclude` キーの値が……

- true
  - その Gist の全ファイルを検索対象外としてスキップ
- ファイル名の配列
  - 名前が一致するファイルを検索対象外としてスキップ
